...
window.csCoderLib = window.csCoderLib || {};
window.csCoderLib.editors = [];

window.csCoderLib.editor = {
    ...
    create: function (id, options) {
        ...
        if (oldEditor !== null) {
            ...
            window.csCoderLib.editors.splice(window.csCoderLib.editors.findIndex(item => item.id === id), 1);
            oldEditor.dispose();
        }
        ...
        window.csCoderLib.editors.push({ id: id, editor: editor });
    },
    ...
    getEditorById: function (id, unobstrusive = false) {
        let editorHolder = window.csCoderLib.editors.find(e => e.id === id);
        if (!editorHolder) {
            if (unobstrusive) return null;
            throw "Couldn't find the editor with id: " + id + " editors.length: " + window.csCoderLib.editors.length;
        }
